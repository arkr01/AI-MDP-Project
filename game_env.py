...
class GameEnv:
    ...
    def check_collision_or_terminal(self, row, col, reward, row_move_dir, col_move_dir):
        terminal = False
        collision = False
    ...
    def check_gem_collected_or_goal_reached(self, row, col, gem_status):
        is_terminal = False
        ...
        if action in self.WALK_ACTIONS:
            if self.grid_data[state.row + 1][state.col] == self.SUPER_CHARGE_TILE:
                ...
                while self.grid_data[next_row + 1][next_col + move_dir] == self.SUPER_CHARGE_TILE:
                    ...
                    next_row, next_col, reward, collision, is_game_over = \
                        self.check_collision_or_terminal(next_row, next_col, reward,
                ...
                next_gem_status, is_solved = self.check_gem_collected_or_goal_reached(next_row, next_col,
                                                                                        next_gem_status)
            ...
            if self.grid_data[state.row + 1][state.col] == self.SUPER_JUMP_TILE:
                ...
                for d in range(move_dist):
                    ...
                    # check for collision or game over
                    next_row, next_col, reward, collision, is_game_over = \
                        self.check_collision_or_terminal(next_row, next_col, reward, row_move_dir=-1, col_move_dir=0)
                    if collision or is_game_over:
                        break
            ...
            for d in range(move_dist):
                ...
                # check for collision or game over
                next_row, next_col, reward, collision, is_game_over = \
                    self.check_collision_or_terminal_glide(next_row, next_col, reward,
                                                             row_move_dir=0, col_move_dir=move_dir)
                if collision or is_game_over:
                    break
            ...
            for d in range(move_dist):
                ...
                # check for collision or game over
                next_row, next_col, reward, collision, is_game_over = \
                    self.check_collision_or_terminal_glide(next_row, next_col, reward, row_move_dir=1, col_move_dir=0)
                if collision or is_game_over:
                    break
